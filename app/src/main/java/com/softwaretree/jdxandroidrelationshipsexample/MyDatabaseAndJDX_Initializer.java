...
public class MyDatabaseAndJDX_Initializer extends DatabaseAndJDX_Initializer {
    ...
    protected void populateDatabase(JDXSetup jdxSetup, boolean newSchemaCreated) throws Exception {
        ...
        try {
            ...
            // Start a new transaction to make all the following statements (until tx_commit) to execute as one unit of operation in the database
            jxSessionHandle.tx_begin();

            // First delete existing SimpleEmp and associated SimpleAddr objects
            jdxHandle.delete2(employeeClassName, "deptId=" + deptId1 + " OR " + "deptId=" + deptId2, JDXS.FLAG_DEEP);

            // Now delete existing SimpleCompany and the associated SimpleForeignLocation and SimpleDept objects
            jdxHandle.delete2(companyClassName, "companyId = " + "'" + companyId + "'", JDXS.FLAG_DEEP);

            // ****************************************************************
            // Now create and insert a new SimpleCompany and 2 new SimpleDept objects in separate calls to JDX
            // ****************************************************************
            SimpleCompany company1 = new SimpleCompany(companyId, "JDX Sample Corporation", "San Jose", "CA", "USA");
            ...
            // Now create a new SimpleCompany object and associated SimpleForeignLocation and SimpleDept objects
            company1 = new SimpleCompany(companyId, "JDXA Sample Corporation", "San Jose", "CA", "USA");
            ...
            // Now insert the newly created SimpleCompany along with the associated SimpleForeignLocation and SimpleDept objects with one JDX call
            ...
            // ****************************************************************
            SimpleAddr addr1 = new SimpleAddr("111 Main Street", null, "San Jose", "CA", "95129", "USA");
            SimpleEmp emp1 = new SimpleEmp(empId1, "John Smith", "Manager", "111-222-3333", (float) 75000.00,
                    dept1, addr1);

            jdxHandle.insert(emp1, JDXS.FLAG_DEEP, null);

            // ****************************************************************
            // Now create and insert multiple SimpleEmp objects along with their associated SimpleAddr 
            // objects in one call to JDX.
            //
            // Because the addrId attribute of the SimpleAddr class is declared as an implicit attribute in 
            // the mapping file, its value for an instance will automatically be initialized by JDX using 
            // the empId value of the containing SimpleEmp object.
            // ****************************************************************
            ArrayList<SimpleEmp> employees = new ArrayList<SimpleEmp>();

            SimpleAddr addr2 = new SimpleAddr("222 State Street", null, "Washington", "D.C.", "20502", "USA");
            SimpleEmp emp2 = new SimpleEmp(empId2, "Mark Clinton", "Engineer", "444-555-6666", (float) 90000.00,
                    dept1, addr2);
            employees.add(emp2);

            SimpleAddr addr3 = new SimpleAddr("333 City Boulevard", "Apartment 33", "San Francisco", "CA",
                    "94103", "USA");
            SimpleEmp emp3 = new SimpleEmp(empId3, "Bob Brown", "Consultant", "777-888-9999", (float) 95000.00,
                    dept2, addr3);
            employees.add(emp3);
